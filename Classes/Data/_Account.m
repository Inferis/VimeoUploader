...
+ (NSArray*)fetchAll:(NSManagedObjectContext*)moc_ {
	NSError *error = nil;
	NSArray *result = [self fetchAll:moc_ error:&error];
	if (error) {
#if TARGET_OS_IPHONE
		NSLog(@"error: %@", error);
#else
...
+ (NSArray*)fetchAll:(NSManagedObjectContext*)moc_ error:(NSError**)error_ {
	...
	NSFetchRequest *fetchRequest = [model fetchRequestFromTemplateWithName:@"all"
													 substitutionVariables:substitutionVariables];
	NSAssert(fetchRequest, @"Can't find fetch request named \"all\".");
